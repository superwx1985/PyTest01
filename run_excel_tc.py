    ...
    try:
        ...
        if level == 0:
            ...
            if config_['A1'] == 'local browser':
                ...
                if isinstance(config_['D1'], (int, float)):
                    ip = str(round(config_['D1']))
                else:
                    ip = config_['D1']
                if isinstance(config_['F1'], (int, float)):
                    port = str(round(config_['F1']))
                else:
                    port = config_['F1']
                if config_['B1'] == 'INTERNETEXPLORER':
        ...
        for row in range(1, data['rows'] + 1):
            ...
            if data['C' + str(row)] in ('action'):
                ...
                if isinstance(data['G' + str(row)], (int, float)):
                    ot = round(data['G' + str(row)])
                elif data['G' + str(row)].isdigit():
                    ot = int(data['G' + str(row)])
                ...
                if isinstance(data['F' + str(row)], (int, float)):
                    data_ = str(round(data['F' + str(row)]))
                else:
                    data_ = data['F' + str(row)]
                ...
                if isinstance(data['G' + str(row)], (int, float)):
                    ...
                    ot = int(data['G' + str(row)])
                try:
                    wait_element.wait_for_text_present(dr, data['F' + str(row)], ot, print_)
                    asserted.append((id_, 'text presented ==> [%s]' % data['F' + str(row)]))
                ...
                if isinstance(data['G' + str(row)], (int, float)):
                    ...
                    ot = int(data['G' + str(row)])
                try:
                    wait_element.wait_for_element_visible(dr, data['D' + str(row)], data['E' + str(row)], ot, print_)
